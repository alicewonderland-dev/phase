            ...
            match super::replacement::replace_event(state, proposed, events) {
                super::replacement::ReplacementResult::Execute(event) => {
                    ...
                    if spell_in_zone(state, entry.id, Zone::Battlefield) {
                        ...
                        // CR 709.5d: the designation follows WHICH HALF was cast
                        // (`room::live_face_door`), not who controls anything — there is
                        // no choice and no legality test here. This argument's only
                        // consumer is the `GameEvent::RoomDoorUnlocked { player_id }`
                        // label.
                        // KNOWN LIMITATION: to the extent that label should follow
                        // anything it is the PERMANENT's controller, which the ETB
                        // `controller_override` path already settles; it is not the
